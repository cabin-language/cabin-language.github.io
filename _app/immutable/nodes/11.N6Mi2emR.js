import"../chunks/CWj6FrbW.js";import"../chunks/wPM4U2Nx.js";import{$ as u,s as e,f}from"../chunks/ChQ8i3mT.js";import{h as b}from"../chunks/3l18Sfkq.js";import{t as m,a as g}from"../chunks/CwLvwI_u.js";import{S as o}from"../chunks/7Ikl-ixd.js";import{T as w}from"../chunks/CU_wdFVd.js";var v=m(`<h1>Control Flow</h1> <p>Control flow in Cabin is mostly handled through labels and <code>is</code> statements. An <code>is</code> statement is like a <code>goto</code> that jumps to a label.</p> <h2 id="labels">Labels &amp; <code>is</code> Statements</h2> <p>When you assign bind a name with a <code>let</code> declaration, the value of the binding is labelled with the name. This might sound a little confusing,
		...
		current expression is being used for a binding. Consider this example from before:</p> <!> <p>So <code>it</code> works its way up the scope tree. In this case, it first checks the <code>if</code> expression. Since the result of the <code>if</code> expression isn't being bound to anything, it goes up one more level to the <code>for</code> loop. The <code>for</code> loop <i>is</i> being bound to a name ( <code>danny</code> ), so <code>it</code> is going to break to this level.</p> <h3 id="return">The <code>return</code> Label</h3> <p>The next reserved label is <code>return</code> . In Cabin, <code>return</code> isn't a keyword with it's own syntax; It's just a built-in identifier. <code>return</code> refers to the current (innermost) action. We haven't talked about <code>actions</code> yet, but if you're familiar with other languages, they're like functions. Here's an example of an
		...
		this:</p> <!> <p>However, this is not the most idiomatic way to use <code>loop</code> ; This example is better fit for <code>it</code> or just <code>danny</code> ; Using <code>loop</code> here actually gives a warning. Instead, <code>loop</code> is useful when we aren't binding the result of the loop:</p> <!> <p>In this case, we want to break out of the loop as soon as we find a person with the first name <code>"danny"</code> .</p> <h3 id="done">The <code>done</code> Value</h3> <p>This is <i>almost</i> idiomatic, except we don't like to use <code>none</code> for this use case, because it's a little unclear that we are just trying to break out of the loop,
		and so using <code>none</code> for this will also give a warning. Instead, we have a built-in alias for <code>none</code> called <code>done</code> that's designed for this exact use case:</p> <!> <p>This provides much more clear intent, and finally we have no warnings; This is the idiomatic way
		to break out of a loop that isn't binding a value. By the way, using <code>done</code> instead of <code>none</code> in any other situation will result in a warning.</p> <h3 id="iteration">The <code>iteration</code> Label</h3> <p>Finally, the <code>iteration</code> label refers to the end of the current loop body, i.e., using <code>iteration is done;</code> is equivalent to a <code>continue</code> in most languages:</p> <!>`,1);function S(h){b(n=>{u.title="Control Flow • Cabin Docs"}),w(h,{page:"Control Flow",children:(n,y)=>{var t=v(),i=e(f(t),8);o(i,{language:"cabin",code:`
